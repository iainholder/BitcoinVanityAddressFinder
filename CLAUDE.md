...
# Run all tests (NUnit, in-project — no separate test project)
dotnet test BitcoinVanityAddressFinder/BitcoinVanityAddressFinder.csproj

# Run a single test by name
dotnet test BitcoinVanityAddressFinder/BitcoinVanityAddressFinder.csproj --filter "FullyQualifiedName~InputStringVerifierTests.IsVanityAddress_WhenContains_Match"
```

## Architecture

**Single-project solution** — everything lives under `BitcoinVanityAddressFinder/`.
...
1. `VanityAddressViewModel.Search()` creates a `CancellationTokenSource` and delegates to `VanityAddressService.Search()`
...
3. Each task checks generated addresses against either `InputStringVerifierService` (string mode) or `DictionaryWordVerifierService` (dictionary mode)
4. `Task.WhenAny` returns the first match; other tasks are cancelled
5. A `DispatcherTimer` (500ms interval) sends attempt counts back to the ViewModel via `WeakReferenceMessenger` using a GUID-based channel token
...
- **"Starts with" skips first character**: Bitcoin addresses have a meaningful first character (network prefix), so starts-with matching uses `address.Remove(0, 1)` to skip it
...
- **Max 8 characters**: The UI enforces a maximum of 8 characters for vanity text because longer searches would take impractically long
